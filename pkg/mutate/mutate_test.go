...
func TestMutateExistingAnnotationsJSON(t *testing.T) {
	rawJSON := `{
...
func TestMutateNoAnnotationsJSON(t *testing.T) {
	rawJSON := `{
		"kind": "AdmissionReview",
		"apiVersion": "admission.k8s.io/v1beta1",
		"request": {
	...
	rr := r.Response
	assert.Equal(t, `[{"op":"add","path":"/metadata/annotations","value":{"cluster-autoscaler.kubernetes.io/safe-to-evict":"true"}}]`, string(rr.Patch))
	assert.Contains(t, rr.AuditAnnotations, "add-eviction-helper")
